...
if (rh.length > 0) {
  ...
  rh.forEach(p => {
    ...
    // RSSI
    const rssiDs = getDataset(rssiChart, p.addr, `NODE ${p.addr}`);
    rssiDs.data.push(p.rssi);

    // SNR
    const snrDs = getDataset(snrChart, p.addr, `NODE ${p.addr}`);
...
if __name__ == "__main__":
    if SERIAL_PORT:
        ...
        print("[LoRa] Running without serial (Azure/dashboard mode)")
