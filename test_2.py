...
BAUD_RATE = 9600
AZURE_INGEST_URL = os.environ.get("AZURE_INGEST_URL")
...
def serial_reader():
    ...
    if not SERIAL_PORT:
        print("[LoRa] No serial port set → skipping serial reader")
        return
    ...
    try:
        ...
        if packet:
            ...
            if AZURE_INGEST_URL:
                try:
                    r = requests.post(AZURE_INGEST_URL, json=packet, timeout=3)
                    print("[AZURE]", r.status_code)
                except Exception as e:
                    print("[HTTP ERROR]", e)
...
if __name__ == "__main__":
    if SERIAL_PORT:
        ...
        print("[LoRa] Running without serial (dashboard only)")
